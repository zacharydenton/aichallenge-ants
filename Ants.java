...
public class Ants {
	...
	public LinkedList<Node> findPath(Tile start, Tile end) {
		...
		frontierSet.add(new Node(this, start, null, end));
		PriorityQueue<Node> frontierQueue = new PriorityQueue<Node>(1, new Comparator<Node>() {
			public int compare(Node a, Node b) {
				return a.value() - b.value();
			}
		});
		frontierQueue.add(new Node(this, start, null, end));
		...
		while (frontierQueue.size() > 0) {
			...
			} else if (frontierSet.size() > 1000) {
				return current.getPath();
			}

			for (Tile neighbor : current.getNeighbors()) {
				if (!frontierSet.contains(neighbor) && !explored.contains(neighbor)) {
					frontierQueue.add(new Node(this, neighbor, current, end));
					frontierSet.add(new Node(this, neighbor, current, end));
				}
			}
		}
	...
	public LinkedList<Node> bestFirstSearch(Tile start, Tile end) {
		PriorityQueue<Node> openQueue = new PriorityQueue<Node>(1, new Comparator<Node>() {
			public int compare(Node a, Node b) {
				return a.value() - b.value();
			}
		});
		openQueue.add(new Node(this, start, null, end));

		HashSet<Node> openSet = new HashSet<Node>(openQueue);
		HashSet<Node> closed = new HashSet<Node>();

		while (openQueue.size() > 0) {
			//Logger.getAnonymousLogger().warning("frontier size: " + open.size());
			Node bestNode = openQueue.remove();
			openSet.remove(bestNode);
			closed.add(bestNode);
			if (bestNode.position.getRow() == end.getRow() && bestNode.position.getCol() == end.getCol()) {
				return bestNode.getPath();
			} else if (openSet.size() > 10) {
				return bestNode.getPath();
			}
			...
			for (Tile neighbor : bestNode.getNeighbors()) {
				...
				if (!seen) {
					Node newNode = new Node(this, neighbor, bestNode, end);
